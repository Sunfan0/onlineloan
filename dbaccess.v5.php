...
//
	...
	function DBGetDataRowsSimple($strSql){//导出数据格式为关联数组
		return DBGetDataRows($strSql , PDO::FETCH_ASSOC);
	}
	
	function DBGetDataRows($strSql , $option = null){//数据格式是索引数组和关联数组都有
...
//myecho($dbh);
	...
	function DBGetDataRowsSimplePrepare($strSql , $arrParas){
		return DBGetDataRows($strSql , PDO::FETCH_ASSOC);//关联数组形式的数据
	}
