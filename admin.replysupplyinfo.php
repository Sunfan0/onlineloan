			  ...
			  <div class="modal-dialog" style="width:1100px;">  
				<div class="modal-content message_align">  
				  <div class="modal-header">  
					...
					<h5 class="modal-title">供方产品回复履历</h5>  
				  </div>  
	...
	<script type="text/javascript">
		...
		function ShowSupplyinfoList(){
			GotListTable = $('#supplyinfolist').DataTable({
				"ajax": {
					"url":"admin.replysupplyinfo.ajax.php?mode=ShowSupplierList",
					"type":"POST"
				},
...
// console.log(json);
		...
		function ShowNewsInfo(id){
			...
			GotInfoTable = $('#NewsInfoTable').DataTable({
				"ajax": {
					"url":"admin.replysupplyinfo.ajax.php?mode=replylist&id="+id,
					"type":"POST"
				},
				//"processing": true,//加载效果
				"serverSide": true,//页面在加载时就请求后台，以及每次对 datatable 进行操作时也是请求后台
				...
				confirm: function(){
					switch(batch){
						case 0:
							...
							url = "admin.replysupplyinfo.ajax.php?mode=Updatereply&type="+type+"&id="+id+"&reason="+reason;
...
console.log(data);
							url = "admin.replysupplyinfo.ajax.php?mode=UpdateMorereply";
							$.get(url,{
