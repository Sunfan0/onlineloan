<?php
	...
	switch($mode){
		...
		case "NewsList":
			...
			$search = Get("search");
			$toLike = $search["value"];
			$strSql .= "where (d.title like '%" . $toLike . "%') or (d.publishname like '%" . $toLike . "%') or (s.username like '%" . $toLike . "%')   ";
		...
		case "replylist":
			...
			$search = Get("search");
			$toLike = $search["value"];
			$strSql .= "and (b.content like '%" . $toLike . "%' or s.username like '%" . $toLike . "%' or d.publishname like '%" . $toLike . "%') ";
			$sqlPara = GetPageParas();
			$strSqlDetail = $strSql . $sqlPara["where"] . $sqlPara["limit"];
			$dataDetail = DBGetDataRowsSimple($strSqlDetail);
