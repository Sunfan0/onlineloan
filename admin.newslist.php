...
?>
		<div id="Maincontainer" class="container" style="margin-top:30px;">
			<form class="navbar-form navbar-left" style="position:relative;top:100px;margin-left: 0;left: 30%;width: 40%;z-index: 500;">
	...
	<script type="text/javascript">
		...
		function BatchOperat(){
			...
			$.confirm({
				...
				cancelButton: '取消',
				confirm: function(){
					url = "admin.newslist.ajax.php?mode=DeleteMore";
					$.get(url,{
...
console.log(json);
		...
		/* function ShowSecondtypelist(parentid){
			url = "admin.newslist.ajax.php?mode=newssecondtypelist&parentid="+parentid;
			$.get(url,function(json,status){
				json = eval("("+json+")");
console.log(json);
				for(i=0;i<json.length;i++){
		...
		function ShowNewsList(type){
			GotListTable = $('#ContentTable').DataTable({
				"ajax": {
					"url":"admin.newslist.ajax.php?mode=ShowNewsList&newstype="+type,
					"type":"POST"
				},
		...
		function ShowNewsInfo(id){
			window.location.href = "admin.news.php?newsId="+id;
		}
   </script> 
</html>