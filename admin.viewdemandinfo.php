<?php	include "header.php";	?>
		...
		<div id="Maincontainer" class="container" style="margin-top:30px;">
			<table id="demandinfolist" class="table table-hover table-bordered">
				<thead>
					<tr>
						<th>ID</th>
						<th>标题</th>
	...
	<script type="text/javascript">
		...
		function BindEvents(){
			$('#demandinfolist tbody').on( 'click', 'td', function () {
		...
		function ShowdemandinfoList(){
			GotListTable = $('#demandinfolist').DataTable({
				"ajax": {
					"url":"admin.viewdemandinfo.ajax.php?mode=ShowSupplierList",
					"type":"POST"
				},
		...
		function ShowDetailInfo(id){
			...
			GotInfoTable = $('#showdetialinfor').DataTable({
				"ajax": {
					"url":"admin.viewdemandinfo.ajax.php?mode=ShowDemanderList&demandinfoid="+id,
					"type":"POST"
				},
				//"processing": true,//加载效果
				"serverSide": true,//页面在加载时就请求后台，以及每次对 datatable 进行操作时也是请求后台
