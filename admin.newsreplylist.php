	...
	<script type="text/javascript">
		...
		function ShowNewsInfoList(){
			GotListTable = $('#NewsInfoList').DataTable({
				"ajax": {
					"url":"admin.newsreplylist.ajax.php?mode=NewsList",
					"type":"POST"
				},
		...
		function ShowNewsInfo(id){
			...
			GotInfoTable = $('#NewsInfoTable').DataTable({
				"ajax": {
					"url":"admin.newsreplylist.ajax.php?mode=replylist&id="+id,
					"type":"POST"
				},
				//"processing": true,//加载效果
				"serverSide": true,//页面在加载时就请求后台，以及每次对 datatable 进行操作时也是请求后台
				...
				"aoColumnDefs": [
					...
					},
					{
						...
						'render': function (data, type, row){
							if(row.zname!=""){
								repliername=row.zname;
							}
							if(row.dname!=""){
								repliername=row.dname;
							}
							if(row.sname!=""){
								repliername=row.sname;
							}
							return repliername;
						}
                    },
...
console.log(json);
				for(var i=0;i<json.length;i++){	
					...
					switch(json[i].isstop){
						case "1":
							isstop = "未屏蔽";
							strbtn = '<button onclick="Updatereply(-1,'+json[i].id+',0)" class="btn btn-danger btn-sm">屏蔽</button>&nbsp;&nbsp;';
							break;
						case "-1":
							isstop = "已屏蔽";
							strbtn = '<button onclick="Updatereply(1,'+json[i].id+',0)" class="btn btn-primary btn-sm">取消屏蔽</button>';
							break;
						case "0":
							isstop = "未处理";
							strbtn = '<button onclick="Updatereply(-1,'+json[i].id+',0)" class="btn btn-danger btn-sm">屏蔽</button>&nbsp;&nbsp;';
				...
				confirm: function(){
					switch(batch){
						case 0:
							...
							url = "admin.newsreplylist.ajax.php?mode=Updatereply&type="+type+"&id="+id+"&reason="+reason;
							$.get(url,function(json,status){
console.log(json);
								switch (json){
									case "1":
...
console.log(data);
							url = "admin.newsreplylist.ajax.php?mode=UpdateMorereply";
							$.get(url,{
