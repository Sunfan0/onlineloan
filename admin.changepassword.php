<?php include "header.php";?>
	...
	<script type="text/javascript">
		...
		function SaveInfo(){
			...
			var newPassword1 = $("#changePassword1").val();
			var newPassword2 = $("#changePassword2").val();
			
			if(oldPassword == ""){$("#currentPassword").focus();CommonWarning('原始密码不能为空。');return;}
			if(newPassword1 == ""){$("#changePassword1").focus();CommonWarning('新密码不能为空。');return;}
			if(newPassword1 != newPassword2){$("#changePassword2").focus();CommonWarning('两次输入的新密码不一致。');return;}
			
			url = "header.php?mode=ChangePassword&oldPassword="+md5(oldPassword)+"&newPassword="+md5(newPassword1);
