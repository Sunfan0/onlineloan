	...
	<script type="text/javascript">
			...
			$('#ContentTable tbody').on( 'click', 'td', function () {
				if ($(this).hasClass('selected') ) {
					...
					if(GotListTable.row('.selected').data()){
						ShowSupplierInfo(GotListTable.row('.selected').data().id);
					}
				}
			});			
		}
		function ShowSupplierlist(){
			GotListTable = $('#ContentTable').DataTable({
				"ajax": {
					"url":"demand.attentionsupplier.ajax.php?mode=ShowSupplierList",
					"type":"POST"
				},
...
console.log(json);
		...
		function ShowSupplierInfo(id){
			url = "demand.attentionsupplier.ajax.php?mode=ShowSupplierDetail&supplierid="+id;
