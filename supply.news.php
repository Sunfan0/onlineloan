...
?>
		...
		<div id="Maincontainer" class="container" style="margin-top:50px;">
			<!--<nav class="navbar navbar-default" role="navigation">
				<div class="container-fluid">
					<form class="navbar-form navbar-right" role="search">
						<a class="btn btn-primary text-right" href="supply.newslist.php"><< 返回</a>
					</form>							
				</div>
			</nav>-->
			<form id="RegisterForm" class="form-horizontal"  style="margin-left:-80px;">
				...
				<div class="form-group">
					<label class="col-sm-2 control-label submitprompt">一级栏目<span>*</span></label>
					<div class="col-sm-10">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<!--<select id="NewsStation" class="selectpicker show-tick form-control" multiple data-live-search="false"></select>-->
						<select class="form-control" id="NewsStation"></select>
					</div>
				</div>
				<div class="form-group">
				...
				<div class="form-group text-right">
					...
					<a href="supply.newslist.php" class="btn btn-default" style="margin-right:15px;">取消</a>
...
console.log(json);
		...
		function ShowSubarealist(){
			...
			$("#NewsStation").html("");
			url = "supply.newslist.ajax.php?mode=subarealist";
			$.get(url,function(json,status){
				json = eval("("+json+")");
console.log(json);
				if(json == null){return;}
		...
		function Supplysubarea(){
			url = "supply.newslist.ajax.php?mode=SupplierSubarea";
			$.get(url,function(json,status){
console.log("SupplierSubarea",json);
				if(json!="")
					$("#NewsStation").val(json);
			});
		}
		...
		function ModifyInfo(){
			...
			url = "supply.newslist.ajax.php?mode=ShownewsDetail&id="+newsId;
			$.get(url,function(json,status){
				json = eval("("+json+")");
console.log(json);
				if(json == null){
				...
				if(json.subarealist == null)
					return;
				$("#NewsStation").val(json.subarealist[0].id);
		...
		function SaveInfo(){
			...
			if(data.content == ""){$("#NewsTitle").focus();CommonWarning('资讯信息不能为空！');return;}
			...
			*/
			...
			if(station == null){
				...
				for(d=0;d<datasubareaall.length;d++){
					if(station == datasubareaall[d].id){
						datasubareaall[d].status = 1;
					}
				}
			}
			...
			//if(data.label == null){$("#NewsLabel").focus();CommonWarning('资讯标签不能为空！');return;}
			
console.log(data);
			
			url = "supply.newslist.ajax.php?mode=UpdateNews";
			$.post(url, {
...
console.log(json);
				switch (json){
					case "1":
						window.location.href = "supply.newslist.php";
						break;
					default:
		...
		function DeleteInfo(){
			$.confirm({
				...
				cancelButton: '取消',
				confirm: function(){
					url = "supply.newslist.ajax.php?mode=Deletenews&id="+newsId;
