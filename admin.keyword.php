<?php	include "header.php";	?>
		...
		<div id="Maincontainer" class="container" style="margin-top:30px;">
			<nav class="navbar navbar-default" role="navigation">
				<div class="container-fluid">
					...
					<form class="navbar-form navbar-right col-sm-3" role="search">
						<button class="btn btn-primary text-right" onclick="AddInfo(); return false;">+ 新增敏感关键字</button>
					</form>
				</div>
			</nav>
			...
			<div class="modal fade" id="KeywordInfo">  
				  ...
				  <div class="modal-body text-center"> 
					  <form class="form-horizontal">
						<div class="form-group">
							<label for="KeywordText" class="col-sm-2 control-label">关键字</label>
							<div class="col-sm-10">
								<input type="text" class="form-control" id="KeywordText" placeholder="敏感关键字">
							</div>
						</div>
					</form>
				  </div>  
				  <div class="modal-footer">
					<button id="DeletKeywordInfo" onclick="DeletInfo(0)" style="margin-right:380px;" type="button" class="btn btn-danger">删除</button>
	...
	<script type="text/javascript">
		...
		function ShowKeywordList(){
			GotListTable = $('#ContentTable').DataTable({
				"ajax": {
					"url":"admin.keyword.ajax.php?mode=KeywordList",
					"type":"POST"
				},
		...
		function ModifyInfo(id,p){
			...
			$('#addTitle').text("修改信息");
			$("#KeywordText").val("");
			$('#KeywordInfo').modal({backdrop: 'static', keyboard: false});
			$("#DeletKeywordInfo").css('display','');
			var keytext = $(p).parent().parent().children("td:eq(1)").html();
			$("#KeywordText").val(keytext);
		}
		function AddInfo(){
			Settings.Flagid = "";
			$('#addTitle').text("填写信息");
			$("#KeywordText").val("");
			$('#KeywordInfo').modal({backdrop: 'static', keyboard: false});
			$("#DeletKeywordInfo").css('display','none'); 
		}
		function DeletInfo(batch){
			...
			switch(batch){
				case 0:
					strcontent = "您确定删除该条敏感关键字？";
					break;
				case 1:
					strcontent = "您确定删除所有选中的敏感关键字？";
					break;
			}
			$.confirm({
				title: '提示',
				...
				confirm: function(){
					switch(batch){
						case 0:
							url = "admin.keyword.ajax.php?mode=DeleteKeyword&id="+Settings.Flagid;
...
console.log(json);
						...
						case 1:
							var data = {};
							data.keywordid = SelectData;
console.log(data);
							url = "admin.keyword.ajax.php?mode=DeleteMore";
							$.get(url,{
...
console.log(json);
		...
		function SaveInfo(){
			...
			url = "admin.keyword.ajax.php?mode=UpdateKeyword";
			$.post(url, {
				flagid : flagid ,
				keyword : keyword
			} ,function(json,status){
